...
<div class="mx-auto max-w-4xl px-4 py-10">
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    @foreach($stats as $item)
      ...
      <div class="bg-base-100 border border-base-300 rounded-2xl p-4 shadow-sm ring-1 {{ $ring }}">
        <div class="flex items-start justify-between gap-3">
          <div>
            <div class="text-sm text-base-content/60">{{ $label }}</div>
            <div class="mt-1 text-3xl font-extrabold text-base-content">{{ $value }}</div>
          </div>

          <div class="flex flex-col items-end gap-2">
            <span class="badge {{ $badge }} badge-outline">{{ $value }}</span>
