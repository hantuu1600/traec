...
<form action="{{ $period->id ? route('admin.periods.update', $period->id) : route('admin.periods.store') }}"
    ...
    <div class="form-control">
        <label class="label px-2"><span class="label-text">Period Name</span></label>
        <input type="text" name="name" value="{{ old('name', $period->name) }}" class="input input-bordered"
            placeholder="Example: Ganjil 2025/2026" required>
    </div>
    ...
    @if(!$period->id)
        <div class="form-control">
            <label class="label cursor-pointer justify-start gap-4">
                ...
                <span class="label-text">Set as Active Period</span>
            </label>
        </div>
    @endif

    <div class="flex justify-end gap-2 mt-4">
        <a href="{{ route('admin.periods.index') }}" class="btn">Cancel</a>
