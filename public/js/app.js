...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (loginForm) {
        ...
        // Live rules
        const validateLoginId = bindLiveValidation(
            loginId,
            loginIdError,
            (v) => {
                if (!v) return false;
        ...
        const validatePassword = bindLiveValidation(
            password,
            passwordError,
            (v) => {
                return v.length >= 8;
        ...
        loginForm.addEventListener("submit", (event) => {
            ...
            if (typeof validateLoginId === "function")
                ok = validateLoginId() && ok;
    ...
    if (registerForm) {
        ...
        // Live rules
        const validateName = bindLiveValidation(
            name,
            errName,
            (v) => v.length >= 3
        );
        const validateEmail = bindLiveValidation(email, errEmail, (v) =>
            isEmail(v)
        );
        const validatePass = bindLiveValidation(
            pass,
            errPass,
            (v) => v.length >= 8
        );
        const validateNidn = bindLiveValidation(
            nidn,
            errNidn,
            (v) => isNumeric(v) && v.length >= 6
        );

        const validateProdi = bindLiveValidation(
            prodi,
            errProdi,
            (v) => v !== ""
        );
        const validateFaculty = bindLiveValidation(
            faculty,
            errFaculty,
            (v) => v !== ""
        );
        const validatePosition = bindLiveValidation(
            position,
            errPosition,
            (v) => v !== ""
        );

        const validatePhone = bindLiveValidation(phone, errPhone, (v) =>
            isValidPhone(v)
        );
        ...
        registerForm.addEventListener("submit", (event) => {
            ...
            if (typeof validateFaculty === "function")
                ok = validateFaculty() && ok;
            if (typeof validatePosition === "function")
                ok = validatePosition() && ok;
...
if (sidebar) {
    ...
    if (isCollapsed) {
        ...
        document.querySelectorAll("#sidebar nav a").forEach((a) => {
            a.classList.add("justify-center", "px-2");
